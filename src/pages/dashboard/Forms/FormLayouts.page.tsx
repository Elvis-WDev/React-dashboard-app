import { Link } from "react-router-dom"
import {
    Breadcrumb,
    SelectBasic,
} from "../../../components"
import { ProductOptions } from "../../../config/Forms/Select/Selects.config"


export const FormLayouts = () => {
    return (
        <>
            <Breadcrumb pageName="Forms / Layouts" />

            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-4 ">
                <div className="flex flex-col gap-4">
                    {/* Product form */}
                    <div className="rounded-lg shadow-default bg-white dark:bg-gray-900">
                        <div className="border-b border-inherit border-gray-100 dark:border-gray-800 py-4 px-6 dark:border-strokedark">
                            <h3 className="font-medium text-black dark:text-white">
                                Product Form
                            </h3>
                        </div>
                        <form action="#">
                            <div className="p-6">
                                <div className="mb-4">
                                    <label className="mb-2 block text-black dark:text-white">
                                        Product category <span className="text-meta-1">*</span>
                                    </label>
                                    <SelectBasic data={ProductOptions} ValueField="id" LabelField="NameProduct" />
                                </div>
                                <div className="mb-4">
                                    <label className="mb-2 block text-black dark:text-white">
                                        Product subcategory <span className="text-meta-1">*</span>
                                    </label>
                                    <SelectBasic data={ProductOptions} ValueField="id" LabelField="NameProduct" />
                                </div>
                                <div className="mb-4">
                                    <label className="mb-2 block text-black dark:text-white">
                                        Product name <span className="text-meta-1">*</span>
                                    </label>
                                    <input
                                        type="text"
                                        placeholder="Product name"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    />
                                </div>

                                <div className="mb-4">
                                    <label className="mb-3 block text-black dark:text-white">
                                        Product description <span className="text-meta-1">*</span>
                                    </label>
                                    <textarea
                                        rows={4}
                                        placeholder="Description..."
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    ></textarea>
                                </div>

                                <div className="mb-4 flex flex-col gap-4 xl:flex-row">
                                    <div className="w-full xl:w-1/2">
                                        <label className="mb-2 block text-black dark:text-white">
                                            Sale price <span className="text-meta-1">*</span>
                                        </label>
                                        <input
                                            type="number"
                                            placeholder="Price"
                                            className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                        />
                                    </div>
                                    <div className="w-full xl:w-1/2">
                                        <label className="mb-2 block text-black dark:text-white">
                                            Buying price <span className="text-meta-1">*</span>
                                        </label>
                                        <input
                                            type="number"
                                            placeholder="Price"
                                            className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                        />
                                    </div>
                                </div>

                                <div className="mb-4">
                                    <label className="mb-2 block text-black dark:text-white">
                                        Stock <span className="text-meta-1">*</span>
                                    </label>
                                    <input
                                        type="number"
                                        placeholder="stock"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    />
                                </div>

                                <div className="mb-4">
                                    <label className="mb-3 block text-black dark:text-white">
                                        Attach file
                                    </label>
                                    <input
                                        type="file"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300 file:mr-4 file:rounded file:border-[0.5px] file:border-stroke file:bg-[#EEEEEE] file:px-2.5 file:text-sm focus:border-primary file:focus:border-primary dark:file:bg-white/30 dark:file:text-white`}
                                    />
                                </div>

                                <button className="flex w-full justify-center border border-gray-200 dark:border-gray-800 bg-indigo-500 dark:bg-indigo-600 bg-primary p-3 font-medium text-white dark:text-white hover:bg-opacity-90">
                                    Register
                                </button>
                            </div>
                        </form>
                    </div>
                    {/* <!-- Contact Form --> */}
                    <div className="rounded-lg shadow-default bg-white dark:bg-gray-900">
                        <div className="border-b border-inherit border-gray-100 dark:border-gray-800 py-4 px-6 dark:border-strokedark">
                            <h3 className="font-medium text-black dark:text-white">
                                Contact Form
                            </h3>
                        </div>
                        <form action="#">
                            <div className="p-6">
                                <div className="mb-4 flex flex-col gap-4 xl:flex-row">
                                    <div className="w-full xl:w-1/2">
                                        <label className="mb-2 block text-black dark:text-white">
                                            First name
                                        </label>
                                        <input
                                            type="text"
                                            placeholder="Enter your first name"
                                            className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                        />
                                    </div>

                                    <div className="w-full xl:w-1/2">
                                        <label className="mb-2 block text-black dark:text-white">
                                            Last name
                                        </label>
                                        <input
                                            type="text"
                                            placeholder="Enter your last name"
                                            className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                        />
                                    </div>
                                </div>

                                <div className="mb-4">
                                    <label className="mb-2 block text-black dark:text-white">
                                        Email <span className="text-meta-1">*</span>
                                    </label>
                                    <input
                                        type="email"
                                        placeholder="Enter your email address"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    />
                                </div>

                                <div className="mb-4">
                                    <label className="mb-2 block text-black dark:text-white">
                                        Subject
                                    </label>
                                    <input
                                        type="text"
                                        placeholder="Select subject"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    />
                                </div>

                                <div className="mb-6">
                                    <label className="mb-2 block text-black dark:text-white">
                                        Message
                                    </label>
                                    <textarea
                                        rows={6}
                                        placeholder="Type your message"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    ></textarea>
                                </div>

                                <button className="flex w-full justify-center border border-gray-200 dark:border-gray-800 bg-indigo-500 dark:bg-indigo-600 bg-primary p-3 font-medium text-white dark:text-white hover:bg-opacity-90">
                                    Send Message
                                </button>
                            </div>
                        </form>
                    </div>
                </div >

                <div className="flex flex-col gap-4">
                    {/* <!-- Sign In Form --> */}
                    <div className="rounded-lg shadow-default bg-white dark:bg-gray-900">
                        <div className="border-b border-inherit border-gray-100 dark:border-gray-800 py-4 px-6 dark:border-strokedark">
                            <h3 className="font-medium text-black dark:text-white">
                                Sign In Form
                            </h3>
                        </div>
                        <form action="#">
                            <div className="p-6">
                                <div className="mb-4">
                                    <label className="mb-2 block text-black dark:text-white">
                                        Email
                                    </label>
                                    <input
                                        type="email"
                                        placeholder="Enter your email address"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    />
                                </div>

                                <div>
                                    <label className="mb-2 block text-black dark:text-white">
                                        Password
                                    </label>
                                    <input
                                        type="password"
                                        placeholder="Enter password"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    />
                                </div>

                                <div className="mt-5 mb-5 flex items-center justify-between">
                                    <label htmlFor="formCheckbox" className="flex cursor-pointer">
                                        <div className="relative pt-0.5">
                                            <input
                                                type="checkbox"
                                                id="formCheckbox"
                                                className="taskCheckbox sr-only"
                                            />
                                            <div className="box mr-3 flex h-5 w-5 items-center justify-center rounded border border-stroke dark:border-strokedark">
                                                <span className="text-white opacity-0">
                                                    <svg
                                                        className="fill-current"
                                                        width="10"
                                                        height="7"
                                                        viewBox="0 0 10 7"
                                                        fill="none"
                                                        xmlns="http://www.w3.org/2000/svg"
                                                    >
                                                        <path
                                                            fillRule="evenodd"
                                                            clipRule="evenodd"
                                                            d="M9.70685 0.292804C9.89455 0.480344 10 0.734667 10 0.999847C10 1.26503 9.89455 1.51935 9.70685 1.70689L4.70059 6.7072C4.51283 6.89468 4.2582 7 3.9927 7C3.72721 7 3.47258 6.89468 3.28482 6.7072L0.281063 3.70701C0.0986771 3.5184 -0.00224342 3.26578 3.785e-05 3.00357C0.00231912 2.74136 0.10762 2.49053 0.29326 2.30511C0.4789 2.11969 0.730026 2.01451 0.992551 2.01224C1.25508 2.00996 1.50799 2.11076 1.69683 2.29293L3.9927 4.58607L8.29108 0.292804C8.47884 0.105322 8.73347 0 8.99896 0C9.26446 0 9.51908 0.105322 9.70685 0.292804Z"
                                                            fill=""
                                                        />
                                                    </svg>
                                                </span>
                                            </div>
                                        </div>
                                        <p>Remember me</p>
                                    </label>

                                    <Link to="#" className="text-sm text-primary hover:underline">
                                        Forget password?
                                    </Link>
                                </div>

                                <button className="flex w-full justify-center border border-gray-200 dark:border-gray-800 bg-indigo-500 dark:bg-indigo-600 bg-primary p-3 font-medium text-white dark:text-white hover:bg-opacity-90">
                                    Login
                                </button>
                            </div>
                        </form>
                    </div>

                    {/* <!-- Sign Up Form --> */}
                    <div className="rounded-lg shadow-default bg-white dark:bg-gray-900">
                        <div className="border-b border-inherit borsder-gray-100 dark:border-gray-800 py-4 px-6 dark:border-strokedark">
                            <h3 className="font-medium text-black dark:text-white">
                                Sign Up Form
                            </h3>
                        </div>
                        <form action="#">
                            <div className="p-6">
                                <div className="mb-4">
                                    <label className="mb-2.5 block text-black dark:text-white">
                                        Name
                                    </label>
                                    <input
                                        type="text"
                                        placeholder="Enter your full name"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    />
                                </div>

                                <div className="mb-4">
                                    <label className="mb-2.5 block text-black dark:text-white">
                                        Email
                                    </label>
                                    <input
                                        type="email"
                                        placeholder="Enter your email address"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    />
                                </div>

                                <div className="mb-4">
                                    <label className="mb-2.5 block text-black dark:text-white">
                                        Password
                                    </label>
                                    <input
                                        type="password"
                                        placeholder="Enter password"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    />
                                </div>

                                <div className="mb-5">
                                    <label className="mb-2.5 block text-black dark:text-white">
                                        Re-type Password
                                    </label>
                                    <input
                                        type="password"
                                        placeholder="Re-enter password"
                                        className={`bg-gray-50 border w-full py-4 px-4 rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:disabled:bg-gray-950 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-600 border-gray-300 ring-gray-300`}
                                    />
                                </div>

                                <button className="flex w-full justify-center border border-gray-200 dark:border-gray-800 bg-indigo-500 dark:bg-indigo-600 bg-primary p-3 font-medium text-white dark:text-white hover:bg-opacity-90">
                                    Register
                                </button>
                            </div>
                        </form>
                    </div>
                </div>
            </div >

        </>
    )
}
